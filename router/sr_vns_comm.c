                         ...
                         char* server)
{
    ...
    if(sr_read_from_server_expect(sr, NULL, VNS_AUTH_REQUEST)!= 1 ||
       sr_read_from_server_expect(sr, NULL, VNS_AUTH_STATUS) != 1)
    ...
    if(strlen(sr->template) > 0)
        if(sr_read_from_server_expect(sr, NULL, VNS_RTABLE) != 1)
